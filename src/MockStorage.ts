...
export class MockStorage {
    ...
    private getETag(blob: string): string {
        if (!this._etags[blob]) {
            this.setETag(blob);
        }
    ...
    private setETag(blob: string): void {
        this._etags[blob] = `${blob}.${this._nextETag++}`;
    }
    ...
    listBlobsSegmented(_: string, token: string | undefined, callback: ErrorFirstFunction<ListResponse>) {
        setTimeout(() => {
            if (!token) {
                token = `unspecified.${this._nextETag++}`;
            }
...
let _mockStorageService: jest.Mock | undefined;

export function initializeStorage(mockStorageService: any) {
    _mockStorageService = mockStorageService as jest.Mock;
}

export function useStorage(storage: FakeStorage, options?: StorageOptions) {
    const fakeService = new MockStorage(storage, options);
    if (!_mockStorageService) {
        throw new Error("To be able to call Mock.useStorage, you must call Mock.initializeStorage(azure.createBlobService) immediately after calling jest.mock(`azure-storage`).");
    }
    _mockStorageService.mockImplementation(() => fakeService);
}
